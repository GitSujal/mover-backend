...
export default function VerificationPage() {
  ...
  const loadPendingVerifications = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await verificationAPI.listPendingVerifications(currentPage, 20);
      ...
      <div className="container mx-auto p-6">
        <Card className="border-destructive">
          <CardContent className="p-6">
            ...
            <Button onClick={loadPendingVerifications} variant="outline" className="mt-4">
              Try Again
            </Button>
          </CardContent>
        </Card>
    ...
    <div className="container mx-auto p-6">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">
          Review and approve pending documents from organizations and drivers
        </p>
      </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                <span>Pending Documents ({verifications?.total || 0})</span>
                <Button onClick={loadPendingVerifications} variant="outline" size="sm">
                  Refresh
                </Button>
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              {verifications?.verifications.length === 0 ? (
                ...
                <>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="space-y-1">
                            ...
                            <p className="text-sm text-muted-foreground">
                              {doc.org_id ? 'Organization' : 'Driver'} Document
                            </p>
                            {doc.document_number && (
                  ...
                  {/* Pagination */}
                  {verifications && verifications.pages > 1 && (
                    <div className="flex items-center justify-between pt-4">
                      ...
                      <span className="text-sm text-muted-foreground">
                        Page {currentPage} of {verifications.pages}
                      </span>
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => setCurrentPage(currentPage + 1)}
                        disabled={currentPage === verifications.pages}
                      >
                        Next
                ...
                <div className="space-y-3">
                  ...
                  {selectedDoc.rejection_reason && (
                    <div>
                      ...
                      <p className="text-sm text-destructive">{selectedDoc.rejection_reason}</p>
                    </div>
                  )}
                </div>
