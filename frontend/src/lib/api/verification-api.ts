...
export const verificationAPI = {
   ...
   */
  getOrganizationStatus: async (orgId: string): Promise<OrganizationVerificationStatus> => {
    const response = await apiClient.get(`/api/v1/verification/organization/${orgId}/status`);
    return response.data;
  },
   ...
   */
  getDriverStatus: async (driverId: string): Promise<DriverVerificationStatus> => {
    const response = await apiClient.get(`/api/v1/verification/driver/${driverId}/status`);
    return response.data;
  },
   ...
   */
  getDocument: async (verificationId: string): Promise<DocumentVerificationResponse> => {
    const response = await apiClient.get(`/api/v1/verification/documents/${verificationId}`);
    return response.data;
  },
