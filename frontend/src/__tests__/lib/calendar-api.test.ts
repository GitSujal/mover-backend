...
describe('Calendar API', () => {
  ...
  describe('getCalendarBookings', () => {
    it('fetches calendar bookings successfully', async () => {
      ...
      const result = await calendarAPI.getCalendarBookings('2025-01-01', '2025-01-31');
  ...
  describe('getDriverSchedule', () => {
    it('fetches driver schedule successfully', async () => {
      ...
      const result = await calendarAPI.getDriverSchedule('driver-1', '2025-01-01', '2025-01-31');

      expect(mockedApiClient.get).toHaveBeenCalledWith(
        expect.stringContaining('/api/v1/calendar/driver/driver-1/schedule')
      );
      expect(result).toEqual(mockResponse);
    });
  });
  ...
  describe('getTruckSchedule', () => {
    it('fetches truck schedule successfully', async () => {
      ...
      const result = await calendarAPI.getTruckSchedule('truck-1', '2025-01-01', '2025-01-31');

      expect(mockedApiClient.get).toHaveBeenCalledWith(
        expect.stringContaining('/api/v1/calendar/truck/truck-1/schedule')
      );
      expect(result).toEqual(mockResponse);
    });
  });
  ...
  describe('getFleetCalendar', () => {
    it('fetches fleet calendar successfully', async () => {
      ...
      const result = await calendarAPI.getFleetCalendar('2025-01-01', '2025-01-31');
  ...
  describe('checkAvailability', () => {
    it('checks availability successfully', async () => {
      ...
      expect(mockedApiClient.post).toHaveBeenCalledWith('/api/v1/calendar/availability', request);
      expect(result).toEqual(mockResponse);
    });
  ...
  describe('Error Handling', () => {
    it('handles API errors', async () => {
      ...
      await expect(calendarAPI.getCalendarBookings('2025-01-01', '2025-01-31')).rejects.toThrow(
        'Network error'
      );
    });
  });
