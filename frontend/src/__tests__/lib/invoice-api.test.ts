...
describe('Invoice API', () => {
  ...
  describe('listOrganizationInvoices', () => {
    it('fetches organization invoices successfully', async () => {
      ...
      expect(mockedApiClient.get).toHaveBeenCalledWith(
        expect.stringContaining('/api/v1/invoices/organization/org-1/list')
      );
    ...
    it('includes status filter in request', async () => {
      ...
      await invoiceAPI.listOrganizationInvoices('org-1', 1, 20, InvoiceStatus.PAID);
  ...
  describe('getInvoice', () => {
    it('fetches invoice details successfully', async () => {
      ...
      expect(mockedApiClient.get).toHaveBeenCalledWith('/api/v1/invoices/invoice-1');
  ...
  describe('createInvoice', () => {
    it('creates invoice successfully', async () => {
      ...
      expect(mockedApiClient.post).toHaveBeenCalledWith('/api/v1/invoices', {
        booking_id: 'booking-1',
        notes: 'Thank you for your business',
      });
  ...
  describe('updateInvoice', () => {
    it('updates invoice status successfully', async () => {
      ...
      expect(mockedApiClient.patch).toHaveBeenCalledWith('/api/v1/invoices/invoice-1', {
        status: InvoiceStatus.CANCELLED,
      });
  ...
  describe('getOrganizationStats', () => {
    it('fetches organization invoice statistics', async () => {
      ...
      expect(mockedApiClient.get).toHaveBeenCalledWith('/api/v1/invoices/organization/org-1/stats');
  ...
  describe('Error Handling', () => {
    it('handles API errors', async () => {
      ...
      await expect(invoiceAPI.listOrganizationInvoices('org-1')).rejects.toThrow('Network error');
    });
  });
});
