...
class UploadURLRequest(BaseModel):
    ...
    max_size_mb: int | None = Field(None, description="Maximum file size in MB", ge=1, le=100)
...
class UploadURLResponse(BaseModel):
    ...
    upload_fields: dict[str, str] = Field(description="Form fields to include in POST request")
...
class UploadCompleteRequest(BaseModel):
    ...
    document_type: str | None = Field(None, description="Type of document for verification")
...
class DownloadURLRequest(BaseModel):
    ...
    file_key: str = Field(description="S3 object key")
    expires_in: int = Field(default=3600, description="URL expiration in seconds", ge=60, le=86400)
